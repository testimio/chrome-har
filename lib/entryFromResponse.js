...
module.exports = function (entry, response, options) {
    ...
    if (response.fromDiskCache || entry.__servedFromCache) {
        entry._fromCache = response.fromDiskCache ? 'disk' : 'memory';
    }
    ...
    setTimings(entry,response);
};

function setTimings(entry, response) {    
    ...
    if (timing) {
        ...
        entry._requestTime = Math.floor(1000 * timing.requestTime) / 1000;
        ...
        if (!entry.__servedFromCache) {
            ...
            const entrySecs = entry.__wallTime + (timing.requestTime - entry.__requestWillBeSentTime);
            if (entrySecs) {
                entry.startedDateTime = dayjs.unix(entrySecs).toISOString();
            }

            if (entry.__requestWillBeSentTime) {
                const blockedQueueing = (entry.__requestWillBeSentTime < timing.requestTime) ? timing.requestTime - entry.__requestWillBeSentTime : -1;
                if (blockedQueueing > 0) {
                    entry.timings._blocked_queueing = blockedQueueing * 1000;
                }
